...
class _SfCalendarState extends State<SfCalendar>
  ...
  Widget _getItem(BuildContext context, int index, bool isRTL) {
    ...
    final DateTime viewMinDate = addDays(_minDate, -_minDate.weekday);
    ...
    if (index > 0) {
      ...
      if (_nextDates.isNotEmpty && index > _nextDates.length - 20) {
        ...
        while (count < 20) {
          for (int i = 1; i <= 100; i++) {
            final DateTime updateDate = addDays(date, i * _kNumberOfDaysInWeek);
        ...
        while (count < 20) {
          for (int i = 1; i <= 100; i++) {
            final DateTime updatedDate =
                addDays(date, -i * _kNumberOfDaysInWeek);
    ...
    final DateTime prevDate = index == 0
        ? _previousDates.isEmpty
            ? addDays(startDate, -_kNumberOfDaysInWeek)
            : _previousDates[0]
        : (index > 0
            ? _nextDates[index - 1]
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    if (_previousDates.isEmpty) {
      ...
      while (count < 50) {
        for (int i = 1; i <= 100; i++) {
          final DateTime updatedDate = addDays(date, -i * _kNumberOfDaysInWeek);
  ...
  void _updateCalendarState(_UpdateCalendarStateDetails details) {
    if (details._currentDate != null &&
      ...
      _currentDate =
          getValidDate(widget.minDate, widget.maxDate, details._currentDate);
      _controller.displayDate = _currentDate;
      details._currentDate = _currentDate;
    }
