...
class _AppointmentLayoutState extends State<_AppointmentLayout> {
  ...
  List<Appointment> _getValidAppointments(
    ...
    for (int i = 0; i < visibleAppointments.length; i++) {
      ...
      if (!isSameDate(
          appointment._actualEndTime, appointment._actualStartTime)) {
        /// Check the span appointment is start after time slot end hour and
        /// end before time slot start hour then skip the rendering.
        if (isSameDate(appointment._actualEndTime,
            appointment._actualStartTime.add(Duration(days: 1)))) {
          final int appointmentStartMinutes =
              (appointment._actualStartTime.hour * 60) +
                  appointment._actualStartTime.minute;
          final int appointmentEndMinutes =
              (appointment._actualEndTime.hour * 60) +
                  appointment._actualEndTime.minute;
      ...
      final int appointmentStartMinutes =
          (appointment._actualStartTime.hour * 60) +
              appointment._actualStartTime.minute;
      final int appointmentEndMinutes = (appointment._actualEndTime.hour * 60) +
          appointment._actualEndTime.minute;
  ...
  void _updateDayAppointmentDetails(List<Appointment> visibleAppointments) {
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      final int totalHours = appointment._actualStartTime.hour - viewStartHour;
      final double mins =
          appointment._actualStartTime.minute - viewStartMinutes;
      ...
      if (yPosition + height <= 0) {
        ...
        /// timeslot end time(Eg., calendar end time is 4 PM and appointment
  ...
  void _updateTimelineMonthAppointmentDetails(
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      final DateTime startTime = appointment._actualStartTime;
      int index = _getVisibleDateIndex(widget.visibleDates, startTime);
      ...
      final DateTime endTime = appointment._actualEndTime;
  ...
  void _drawMonthAppointmentView(Canvas canvas, Size size, double cellWidth,
    ...
    for (int i = 0; i < appointmentCollection.length; i++) {
      ...
      if (appointmentView.position < maximumDisplayCount ||
        ...
        final double yPosition = appointmentView.appointmentRect.top +
            ((appointmentView.appointmentRect.height - _textPainter.height) /
                2);
